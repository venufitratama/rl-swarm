...
install_localtunnel() {
    if command -v lt >/dev/null 2>&1; then
        echo_green ">> Localtunnel is already installed."
        return 0
    fi
    echo_green ">> Installing localtunnel..."
    npm install -g localtunnel > /dev/null 2>&1
    if [ $? -eq 0 ]; then
        echo_green ">> Localtunnel installed successfully."
        return 0
    else
...
start_localtunnel() {
    ...
    if [ -n "$URL" ]; then
        ...
        echo_green ">> Success! Please visit this website: ${URL}"
        echo_green ">> Then enter this password: ${PASS} to access the website and log in using your email."
        return 0
    else
...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    if install_localtunnel && start_localtunnel; then
        echo_green ">> Using localtunnel for remote access"
    else
        # Fallback to localhost if localtunnel fails
        if [ -z "$DOCKER" ]; then
